...
- FastAPI 请求体必须使用明确的 Pydantic Schema，不使用无边界的通用字典模型承接写接口。
- 请求模型默认禁止多余字段；除非有明确兼容理由，不允许使用 `extra="allow"`。
- 前端新增或调整提交字段时，必须同步更新：
  - Pydantic Schema
  - API 路由
  - 业务模块
  - 相关测试或手动验证说明
- 工作台类接口应按业务动作拆分请求模型，例如任务、风险、Due Date、交付物确认，不再共用一个宽松 mutation 模型。
- FastAPI 入口必须保留统一请求日志，至少记录：
  - method
  - path
  - status
  - duration
- 4xx/5xx 请求应能从日志定位到具体接口；未捕获异常必须记录堆栈。
- 文件系统操作必须记录可排查上下文：
  - project_id 或 task_id
  - 源路径
  - 目标路径或目标目录
  - 原始异常
- 面向前端的错误信息要清晰可读；日志中保留完整异常，便于排查 NAS、UNC 共享盘、权限不足、文件占用和网络闪断。
- 修改扫描、导入、上传、目录迁移、回收站归档时，不允许只吞异常或只返回笼统失败。
...
- 修改后端、前端、静态资源、API、启动入口或会影响浏览器测试的功能后，应在验证通过后自动重启本地服务，便于用户直接刷新测试。
- 默认重启方式：
...
- 如果旧服务占用端口，应先定位并停止旧进程，再启动新服务；不能安全停止时，必须明确说明当前阻塞和需要用户确认的动作。
- 重启后应提示用户使用当前测试地址：
...
- 后续每次发现、修复项目库相关 bug，都要追加记录现象、根因、修复、验证和剩余风险。
