...
- 当前服务器端 `打开文件夹` 只适用于明确的本机模式。LAN 第一版改为显示并复制 UNC 路径，不允许 Web 请求在服务器上启动 Explorer。
- 上传限制采用配置化策略：默认单文件上限 500 MB；PDF、DOCX、XLSX 等文档解析使用更小的独立上限；3D 模型等大文件可以归档但不强制解析。实施前用真实文件样本校准默认值。
