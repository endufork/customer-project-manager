...
| 编号 | 字段 | 是否必填 | 是否唯一 | 说明 |
|---|---|---|---|---|
| 系统临时项目号 | `intake_no` | 是 | 是 | 系统自动生成，项目创建时立即可用 |
| 内部项目设备号 | `equipment_no` | 否 | 填写后唯一 | 公司内部正式设备号，可能后续补充 |
...
1. 如果已有内部项目设备号，使用 `equipment_no`。
2. 如果暂时没有内部项目设备号，使用系统临时项目号 `intake_no`。
...
如果项目创建后才补充内部项目设备号，第一阶段建议默认不自动重命名已经创建的文件夹，只在项目详情页提示：

```text
已补充内部项目设备号，是否将项目文件夹从临时项目号重命名为正式设备号？
```
...
| code | name |
|---|---|
| no_equipment_no | 待补内部设备号 |

当项目还没有内部设备号时，可使用该状态，或继续使用 `询价录入` 并通过“设备号为空”筛选。
...
|---|---|---|---|
| id | TEXT | 是 | 系统主键 |
| intake_no | TEXT | 是 | 系统临时项目号，唯一 |
| equipment_no | TEXT | 否 | 内部项目设备号，填写后唯一 |
...
2. 如果补充内部项目设备号，是否需要自动重命名项目文件夹。
3. 备份目录最终放移动硬盘、NAS 还是网盘同步目录。
4. 是否需要支持更多币种，例如 `EUR`、`JPY`。
